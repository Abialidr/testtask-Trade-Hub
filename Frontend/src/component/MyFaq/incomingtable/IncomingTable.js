...
const IncomingTable = () => {
  ...
  const { handleIncomingList, incomingList } = useIncomingList();
  const [pageNumber, setPageNumber] = useState(0);
  const navigate = useNavigate();
  const usersPerPage = 10;
  const pageVisited = pageNumber * usersPerPage;
  const displayUsers = incomingList.slice(
    pageVisited,
    pageVisited + usersPerPage
  );
  const pageCount = Math.ceil(incomingList.length / usersPerPage);
  const handleChangePage = ({ selected }) => {
    setPageNumber(selected);
  };
  return (
    <>
      <div className="h-full overflow-x-auto outgoing relative">
        <table className=" pl-8  w-full">
          <thead>
          ...
          <tbody>
            {displayUsers.map((data, i) => {
              const dateCreated = moment(data.createdAt).format('l');
              const dueDate = moment(data.deliveryDate).format('l');
              return (
                <tr
                  className="h-[60px] table-popup"
                  key={i}
                  onClick={() => {
                    navigate(`/IncomingView?id=${data._id}`);
                  }}
                >
                  <td className="font-medium pl-8 text-sm text-dark-gray">
                    {data.title}
                  </td>
                  <td className="font-medium text-sm text-dark-gray">
                    {data.company.name}
                  </td>
                  ...
                  >
                    <div className="flex text-center items-center">
                      ...
                      <div className="ml-auto mr-14 cursor-pointer select-none relative">
                        {opneModal === i && (
                          <div className="modal py-[6px] popup-main -right-10 z-10 absolute w-[192px] bg-white shadow-lg">
                            <ul>
